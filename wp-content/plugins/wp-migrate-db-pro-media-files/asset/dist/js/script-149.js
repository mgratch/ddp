...
(function ($, wpmdb) {
	...
	if (!Object.keys) {
		Object.keys = function () {
			...
			return function (obj) {
				if ('object' !== (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) && ('function' !== typeof obj || null === obj)) {
	...
	var disable_media_files_option = function disable_media_files_option() {
	...
	var hide_show_options = function hide_show_options(unavailable) {
	...
	function migrate_media_files_recursive(args) {
		...
		$.each(args.files_to_migrate, function (index, value) {
			if ('push' === wpmdb.functions.wpmdb_migration_type() && value > remote_max_upload_size) {
				...
				if (file_chunk_size + value > args.bottleneck || number_of_files_to_migrate >= wpmdb.mediaFiles.remote_connection_data.media_files_max_file_uploads) {
					return false;
				} else {
					file_chunk_to_migrate.push(index);
					file_chunk_size += value;
	...
	function maybe_show_data_and_files_differ_notice() {
		...
		if ('true' === wpmdb_data.is_multisite && $mf_select_subsites.is(':checked') && undefined !== selected_subsites && null !== selected_subsites && undefined !== selected_tables && null !== selected_tables && 0 < selected_tables.length) {
