...
class DUP_PRO_Settings_U
{
	...
	public function run_export()
	{
		...
		$crypt = new DUP_PRO_Crypt();
		
		$encrypted_data = $crypt->encrypt('test', $json_file_data);
	...
	public function run_import($filename, &$opts)
	{
		...
		$crypt = new DUP_PRO_Crypt();
		$json_data = $crypt->decrypt('test', $encrypted_data);
		...
		foreach($opts as $import_type)
		{
			switch($import_type)
			{
				case 'schedules':					
					...
					DUP_PRO_U::log_object("#### storage map=", $storage_map);
	...
	private function import_schedules(&$import_data, &$storage_map, &$template_map)
	{
		...
		foreach($import_data->schedules as $schedule_data)
		{
			...
			for($i = 0; $i < count($schedule->storage_ids); $i++)
			{
				...
				DUP_PRO_U::log("#### mapped $old_storage_id to {$schedule->storage_ids[$i]}");
			}
