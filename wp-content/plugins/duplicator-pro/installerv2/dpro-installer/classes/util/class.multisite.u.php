	...
	{		
		...
		{			
			...
			foreach($subsite_ids as $subsite_id)
			{
				...
				{
					$sql = "DELETE FROM $usermeta_table_name WHERE meta_key like '{$subsite_prefix}%'";
			...
			if($retained_subsite_id != 1)
			{
				...
				$sql = "DELETE FROM $usermeta_table_name WHERE meta_key LIKE '$base_prefix%' AND meta_key NOT LIKE '$retained_subsite_prefix%'";
		...
		{		
			...
			{ 				
				if(($subsite_id != $retained_subsite_id) && ($subsite_id > 1))
				{
					...
					DUPX_Log::Info("#### subsite prefix $subsite_prefix");
					
					$subsite_table_names = DUPX_DB_U::query_column_to_array($dbh, "SHOW TABLES LIKE '{$subsite_prefix}%'");
