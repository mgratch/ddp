...
if (isset($_GET['dbtest'])) 
{
	...
	else 
	{
		...
		$dbvar_version = DUPX_U::mysql_version($dbConn);
...
DUPX_Log::Info("INSTALLER DIR 755:\t" . var_export($GLOBALS['CHOWN_INSTALLER_DIRECTORY'], true));
   ...
   // $archive_name = $_POST['package_name'];
    ...
    if($_POST['archive_engine'] == 'shellexec_unzip')
    {
        ...
        if($stderr != '')
        {
			...
			$zip_error_message .= "<br/><br/><strong>To resolve error see <a href='https://snapcreek.com/duplicator/docs/faqs-tech/#faq-installer-130-q' target='_blank'>https://snapcreek.com/duplicator/docs/faqs-tech/#faq-installer-130-q</a></strong>";
...
if ($_POST['dbaction'] != 'manual')
{
	...
	DUPX_U::mysql_set_charset($dbh, $_POST['dbcharset'], $_POST['dbcollate']);
	...
	$dbvar_maxtime		= DUPX_U::mysql_variable_value($dbh, 'wait_timeout');
	$dbvar_maxpacks		= DUPX_U::mysql_variable_value($dbh, 'max_allowed_packet');
	$dbvar_sqlmode		= DUPX_U::mysql_variable_value($dbh, 'sql_mode');
	$dbvar_version		= DUPX_U::mysql_version($dbh);
