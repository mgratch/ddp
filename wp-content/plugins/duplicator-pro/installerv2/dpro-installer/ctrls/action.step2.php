	...
	foreach($ac->subsites as $subsite)
	{
		...
		{					
			...
			if($GLOBALS['MU_MODE'] == DUPX_Multisite_Mode::Subdomain)
			{
				...
				array_push($GLOBALS['REPLACE_LIST'], array('search' => $old_subdomain, 'replace' => $_POST['url_new']));
				array_push($GLOBALS['REPLACE_LIST'], array('search' => urlencode($old_subdomain), 'replace' => urlencode($_POST['url_new'])));
			} 
